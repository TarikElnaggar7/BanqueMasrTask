...
@Component({
  selector: 'app-euro-gbp',
  templateUrl: './euro-gbp.component.html',
  styleUrls: ['./euro-gbp.component.scss'],
})
export class EuroGbpComponent implements OnInit {
  ...
  constructor(
    ...
    translate.onLangChange.subscribe((event: LangChangeEvent) => {
      ...
      this.statisticalData[this.FIRST_ITEM].name =
        translate.instant('main.tables.lowest');
      this.statisticalData[this.SECOND_ITEM].name = translate.instant(
        'main.tables.highest'
      );
      this.statisticalData[this.THIRD_ITEM].name = translate.instant(
        'main.tables.average'
      );
    });
  }
  ...
  ngOnInit() {
    ...
    if (this.currencyExchangeService.isServiceReferral) {
      ...
      this.checkValidityOnLoad(
        this.converterForm.controls['amountControl'].value,
        'amountControl'
      );
      this.checkValidityOnLoad(
        this.converterForm.controls['toControl'].value,
        'toControl'
      );
      this.checkValidityOnLoad(
        this.converterForm.controls['fromControl'].value,
        'fromControl'
      );
    ...
    this.converterForm.get('fromControl').setValue('EUR');
    this.converterForm.get('toControl').setValue('GBP');
  }
  ...
  setFormValidity() {
    ...
    this.currencyExchangeService.isValid =
      isAmount &&
      this.checkForMatchedInputValue('fromControl') &&
      this.checkForMatchedInputValue('toControl');
  }
  ...
  checkForMatchedInputValue(formControlName: string): boolean {
    ...
    return (
      this.mappedCurrencies.filter(
        (currency) => currency === controlValue.toUpperCase()
      ).length !== 0
    );
  }
  ...
  checkValidityOnLoad(value: string, inputName: string) {
    const matchedCurrency =
      this.mappedCurrencies.filter((currency) => currency.includes(value))[
        this.FIRST_ITEM
      ] || ''.toString();
  ...
  exchangeRates(): void {
    ...
    this.fromCurrency = this.filterSelectedValue(
      FormNames.FromControl
    ).currency;
  ...
  getToValueChanges(stringValue: string): Observable<string[]> {
    return this.converterForm.get(stringValue).valueChanges.pipe(
      startWith(''),
      map((value) =>
        this.filterInputValue(value, this.currencyExchangeService.toCurrencies)
      )
    );
  }
  ...
  calculateExchangeRate(): string {
    return (
      (this.converterForm.get(FormNames.AmountControl).value * this.toRate) /
      this.fromRate
    ).toFixed(3);
  }
  ...
  filterTableUponDaySelection(
    ...
    return this.currencyExchangeService.periodicHistoryExchangeRates.filter(
      (item?) => {
        if (
          Math.abs(+item.creationDate.split('/')[1] - +date.split('/')[1]) === 1
        ) {
          return (
            Math.abs(+item.creationDate.split('/')[0] - +date.split('/')[0]) >=
            30 - dayInterval
          );
        } else if (
          Math.abs(+item.creationDate.split('/')[1] - +date.split('/')[1]) === 0
        ) {
          return Math.abs(
            +(
              Math.abs(
                +item.creationDate.split('/')[0] - +date.split('/')[0]
              ) <= dayInterval
            )
          );
        }
    ...
    return this.currencyExchangeService.periodicHistoryExchangeRates.filter(
      (item) => {
        return (
          Math.abs(+item.creationDate.split('/')[0] - +date.split('/')[0]) <=
            dayInterval &&
          Math.abs(+item.creationDate.split('/')[1] - +date.split('/')[1]) <=
            monthInterval
        );
      }
    );
  ...
  calculateStatisticsFromNewArray(
    newDataTableArray: PeriodicHistoryElement[]
  ): void {
    this.statisticalData = [
      ...
      },
      {
        ...
        summary:
          this.getSortedRate(newDataTableArray)[newDataTableArray.length - 1],
      },
      {
      ...
      this.apiRequestService.getExchangeRates(Currency.USD).subscribe(
        (exchangeRate: ExchangeRatesResponse): void => {
          this.currencyExchangeService.exchangeRates =
            this.mapResponseData(exchangeRate);

          this.currencyExchangeService.fromCurrencies =
            this.mapItemCurrencies();
...
\n${(this.toRate / this.fromRate).toFixed(5)}`,
  ...
  getStatisticalDataValues(): Statistics[] {
    ...
    return (this.statisticalData = [
      ...
      },
      {
        name: StatisticalDataTableFields.Highest,
        summary:
          this.getSortedRate(periodicHistoryArray)[
            periodicHistoryArray.length - 1
          ],
      },
      {
        name: StatisticalDataTableFields.Average,
        summary:
          this.getAverageRate(periodicHistoryArray) > -1
            ? this.getAverageRate(periodicHistoryArray)
            : 0,
      },
    ]);
  }
  ...
  private filterInputValue(
    value: string,
    arrayGoingFiltered: string[]
  ): string[] {
    const filterValue = value.toLowerCase();

    return arrayGoingFiltered.filter((option) =>
